...
DimCustomer AS (
    SELECT *
    FROM LIVE.dim_customer
),
DimPaymentTerms AS (
    SELECT *
    FROM LIVE.dim_payment_terms
    WHERE acct_type_s = 'D'
      AND ifnull(slt_delete, '') <> 'X'
      AND active_flag_s = 'Y'
),
...
DimBuHierarchy AS (
    SELECT *
    FROM fdh.slt_raw.dim_bu_heirarchy
    WHERE active_flag = 'Y'
),
